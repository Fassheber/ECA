...
class beneficiariesDAO
{
    ...
    public function salvar($beneficiaries)
    {
        ...
        try {
            if ($beneficiaries->getIdBeneficiaries() != "") {
                ...
                $statement = $pdo->prepare("INSERT INTO tb_beneficiaries (str_nis, str_name_person, str_cpf, int_rgp)
                                                      VALUES (:str_nis, :str_name_person, :str_cpf, :int_rgp)");
            }
    ...
    public function atualizar($beneficiaries)
    {
        global $pdo;
        try {
            $statement = $pdo->prepare("SELECT id_beneficiaries, str_nis, str_name_person, str_cpf, int_rgp
            FROM tb_beneficiaries WHERE id_beneficiaries = :id");
            $statement->bindValue(":id", $beneficiaries->getIdBeneficiaries());
            if ($statement->execute()) {
